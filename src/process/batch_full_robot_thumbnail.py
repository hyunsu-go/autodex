...
LOCAL_CACHE = Path.home() / "cache" / "batch_full_robot_thumbnail"
...
THUMB_SCRIPT = Path.home() / "AutoDex" / "src" / "visualization" / "overlay_full_robot_thumbnail_single.py"
...
def upload_dir(local_dir, nas_dir):
    ...
    for item in local_dir.rglob("*"):
        ...
        shutil.copy2(str(item), str(dst))  # overwrite
...
def run_thumbnail(local_ep, nas_ep, obj_name, hand, candidate_dir, local_out, frame_pbar=None):
    ...
    cmd = [
        ...
        "--arm_state", str(nas_ep / "arm" / "state.npy"),
        "--hand_state", str(nas_ep / "hand" / "state.npy"),
    ...
    for obj in objects:
        ...
        for ep in sorted(obj_dir.iterdir()):
            ...
            if not (ep / "raw" / "timestamps" / "timestamp.npy").exists():
                continue
            if not (ep / "arm" / "state.npy").exists():
                continue
            if not (ep / "hand" / "state.npy").exists():
                continue
            if not (ep / "C2R.npy").exists():
                continue
            serials = sorted(p.stem for p in (ep / "videos").glob("*.avi"))
            if serials:
                eps.append((ep.name, serials, None))
        if eps:
            work.append((obj, eps))
    return work
...
def main():
    ...
    def needs_work(obj, ep, serials):
        return True  # always regenerate with new overlay
    ...
    for ei, (obj, ep, serials, cand) in enumerate(flat):
        ...
        # always regenerate with new overlay; no skip.
